...
function bindEvents() {
  ...
  els.cropCanvas.addEventListener('wheel', handleWheelZoom, { passive: false });

  els.zoomSlider.addEventListener('input', (e) => zoomCropEngine(parseFloat(e.target.value)));
  els.btnZoomIn.addEventListener('click', () => zoomCropEngine(state.cropEngine.scale * 1.15));
  els.btnZoomOut.addEventListener('click', () => zoomCropEngine(state.cropEngine.scale * 0.85));
  els.btnFitImage.addEventListener('click', resetCropEngine);
  els.btnUndoCrop.addEventListener('click', undoCropState);
  els.btnResetCrop.addEventListener('click', resetCropEngine);
  els.btnCancelCrop.addEventListener('click', closeCropModal);
  els.btnApplyCrop.addEventListener('click', executeCrop);

  // Transform Toolbar Events
  document.querySelectorAll('.tool-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      const side = e.currentTarget.dataset.side;
      const action = e.currentTarget.dataset.action;
      handleTransformAction(side, action);
    });
  });

  // A4 & Download Actions
  els.btnPrepareA4.addEventListener('click', generateA4Layout);
  els.btnDownloadPNG.addEventListener('click', downloadPNG);
  els.btnDownloadPDF.addEventListener('click', downloadPDF);

  // Window Resize
  window.addEventListener('resize', () => {
    if (!els.cropModalOverlay.classList.contains('is-hidden')) {
      resizeCropCanvas();
      renderCropCanvas();
    }
  });
  ...
  window.addEventListener('keydown', (e) => {
    if (els.cropModalOverlay.classList.contains('is-hidden')) return;
    if (e.key === 'Escape') closeCropModal();
    if (e.key === 'Enter') executeCrop();
    if (e.key === '+' || e.key === '=') zoomCropEngine(state.cropEngine.scale * 1.1);
    if (e.key === '-') zoomCropEngine(state.cropEngine.scale * 0.9);
